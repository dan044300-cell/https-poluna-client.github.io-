...
// Авто-выбор сервера: на локальной разработке — localhost, на хостинге — онлайн-API.
const SERVER_URL = (location.hostname !== 'localhost' && location.hostname !== '127.0.0.1')
    ? 'https://poluna-server.onrender.com'
    : 'http://localhost:3000';

async function syncServer(path, payload) {
    try {
    ...
    } catch (e) { /* сервер недоступен — работаем локально */ }
}
...
// Launcher download — subscribers only
function downloadLauncher() {
    ...
    if (!user.sub || subLabel(user) === 'free') {
        alert('Лаунчер доступен подписчикам. Оформи подписку — ключ откроет скачивание.');
        switchModal('profile', 'purchase');
        return;
    }
    window.location.href = 'launcher/Launcher.exe';
}
...
async function handleRegister(event) {
    ...
    syncServer('/api/register', { nick, email, password: pass });
...
function handlePurchase(event) {
    ...
    // Сервер выдаёт ключ из пула и привязывает к аккаунту.
    // Если сервер недоступен — генерируем локально (демо).
    const finalize = (key) => {
        users[idx].sub = { plan, expiresAt, key };
        dbSaveUsers(users);
        renderAuth();
    ...
    fetch(SERVER_URL + '/api/purchase', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: user.emailLower, plan })
    })
        .then(r => r.json())
        .then(j => {
            if (j && j.ok && j.key) finalize(j.key);
            else finalize(genLicenseKey(users));
        })
        .catch(() => finalize(genLicenseKey(users)));
}
...
// =========================================================
// GITHUB — demo auth (полный OAuth требует сервер)
// =========================================================
...
// Real redirect to GitHub (works only if Client ID is set)
function githubRedirect() {
    if (!GITHUB_CLIENT_ID || GITHUB_CLIENT_ID.indexOf('YOUR_') === 0) {
        alert('Сначала впиши Client ID в script.js (константа GITHUB_CLIENT_ID) и создай OAuth App на github.com/settings/developers. В Authorization callback URL укажи: ' + GITHUB_REDIRECT_URI);
        return;
    }
...
function openGithub() {
    const box = document.getElementById('githubCodeBox');
    if (box) {
        const code = localStorage.getItem('poluna_github_code');
        box.style.display = code ? 'block' : 'none';
        const span = document.getElementById('githubCodeSpan');
        if (span) span.textContent = code || '';
    }
    const err = document.getElementById('githubError');
    if (err) err.textContent = '';
    openModal('github');
}

async function handleGithubRegister(event) {
    event.preventDefault();
    const error = document.getElementById('githubError');
    error.textContent = '';

    const nick = document.getElementById('ghNick').value.trim();
    const email = document.getElementById('ghEmail').value.trim().toLowerCase();
    const pass = document.getElementById('ghPass').value;
    const pass2 = document.getElementById('ghPass2').value;
    const profile = document.getElementById('ghProfile').value.trim();
    ...
    if (dbFindByEmail(email)) {
        error.textContent = 'Эта почта уже занята.';
        return;
    }
    if (dbGetUsers().some(u => u.nick.toLowerCase() === nick.toLowerCase())) {
        error.textContent = 'Этот ник уже занят.';
        return;
    }
    if (pass.length < 8 || !/[A-Za-zА-Яа-яЁё]/.test(pass) || !/[0-9]/.test(pass)) {
    ...
    users.push({
        ...
        code: localStorage.getItem('poluna_github_code') || '',
        createdAt: new Date().toISOString()
    });
    dbSaveUsers(users);
    localStorage.setItem(DB_SESSION, email);
    localStorage.removeItem('poluna_github_code');

    renderAuth();
    closeModal('github');
    openWelcome(dbFindByEmail(email), pass);
}

function handleGithubLogin() {
    const error = document.getElementById('githubError');
    error.textContent = '';
    const input = document.getElementById('ghEmail');
    const email = input ? input.value.trim().toLowerCase() : '';
    if (!email) {
        error.textContent = 'Введи почту GitHub-аккаунта выше.';
        return;
    }
    const user = dbFindByEmail(email);
    if (user && user.via === 'github') {
        localStorage.setItem(DB_SESSION, email);
        renderAuth();
        closeModal('github');
        alert('С возвращением, ' + user.nick + '!');
    } else if (user) {
        error.textContent = 'Этот аккаунт создан обычной регистрацией — войди с паролем.';
    } else {
        error.textContent = 'GitHub-аккаунт с такой почтой не найден.';
    }
}

// If the browser returned from GitHub with ?code=... — store it and open demo modal
(function detectGithubCallback() {
    const params = new URLSearchParams(window.location.search);
    if (params.has('code')) {
        localStorage.setItem('poluna_github_code', params.get('code'));
        window.history.replaceState(null, '', window.location.pathname);
        setTimeout(openGithub, 0);
    }
})();
